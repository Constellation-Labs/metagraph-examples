...
object Main
  ...
  override def dataApplication: Option[BaseDataApplicationL1Service[IO]] = Option(BaseDataApplicationL1Service(new DataApplicationL1Service[IO, Usage, State] {
    ...
    override def validateData(oldState: State, updates: NonEmptyList[Signed[Usage]])(implicit context: L1NodeContext[IO]): IO[DataApplicationValidationErrorOr[Unit]] = Data.validateData(oldState, updates)

    override def validateUpdate(update: Usage)(implicit context: L1NodeContext[IO]): IO[DataApplicationValidationErrorOr[Unit]] = Data.validateUpdate(update)

    override def combine(oldState: State, updates: NonEmptyList[Signed[Usage]])(implicit context: L1NodeContext[IO]): IO[State] = Data.combine(oldState, updates)
  }))
}
