...
object StateUpdateCombiner {
  ...
  def make[F[_]: Async: SecurityProvider: Hasher]: StateUpdateCombiner[F, TX, DS] =
    new StateUpdateCombiner[F, TX, DS] {
      ...
      private def modifyTask(update: Signed[Updates.ModifyTask], inState: DS, ctx: L0NodeContext[F]): F[DS] =
          for {
            currentOrdinal <- ctx.getLastCurrencySnapshot
              .map(_.map(_.signed.value.ordinal).getOrElse(SnapshotOrdinal.MinValue))
              .map(_.next)
      ...
      private def completeTask(update: Signed[Updates.CompleteTask], inState: DS, ctx: L0NodeContext[F]): F[DS] =
          for {
            currentOrdinal <- ctx.getLastCurrencySnapshot
              .map(_.map(_.signed.value.ordinal).getOrElse(SnapshotOrdinal.MinValue))
              .map(_.next)
      ...
      private def removeTask(update: Signed[Updates.RemoveTask], inState: DS, ctx: L0NodeContext[F]): F[DS] =
          for {
            currentOrdinal <- ctx.getLastCurrencySnapshot
              .map(_.map(_.signed.value.ordinal).getOrElse(SnapshotOrdinal.MinValue))
              .map(_.next)
