...
import com.my.nft_example.l0.custom_routes.CustomRoutes.{getAllCollections, getAllCollectionsOfAddress, getAllNFTsOfAddress, getCollectionById, getCollectionNFTById, getCollectionNFTs}
...
object Main
  ...
  def dataApplication: Option[BaseDataApplicationL0Service[IO]] =
    Option(BaseDataApplicationL0Service(new DataApplicationL0Service[IO, NFTUpdate, NFTUpdatesState, NFTUpdatesCalculatedState] {
      override def genesis: DataState[NFTUpdatesState, NFTUpdatesCalculatedState] = DataState(NFTUpdatesState(List.empty), NFTUpdatesCalculatedState(Map.empty))

      override def validateData(state: DataState[NFTUpdatesState, NFTUpdatesCalculatedState], updates: NonEmptyList[Signed[NFTUpdate]])(implicit context: L0NodeContext[IO]): IO[DataApplicationValidationErrorOr[Unit]] = shared_data.Main.validateData(state, updates)

      override def validateUpdate(update: NFTUpdate)(implicit context: L0NodeContext[IO]): IO[DataApplicationValidationErrorOr[Unit]] = IO {
        ().validNec
      }

      override def combine(state: DataState[NFTUpdatesState, NFTUpdatesCalculatedState], updates: List[Signed[NFTUpdate]])(implicit context: L0NodeContext[IO]): IO[DataState[NFTUpdatesState, NFTUpdatesCalculatedState]] = shared_data.Main.combine(state, updates)
      ...
      override def getCalculatedState(implicit context: L0NodeContext[IO]): IO[(SnapshotOrdinal, NFTUpdatesCalculatedState)] = CalculatedState.getCalculatedState

      override def setCalculatedState(ordinal: SnapshotOrdinal, state: NFTUpdatesCalculatedState)(implicit context: L0NodeContext[IO]): IO[Boolean] = CalculatedState.setCalculatedState(ordinal, state)

      override def hashCalculatedState(state: NFTUpdatesCalculatedState)(implicit context: L0NodeContext[IO]): IO[Hash] = CalculatedState.hashCalculatedState(state)
    }))

  def rewards(implicit sp: SecurityProvider[IO]): Option[Rewards[IO, currency.CurrencySnapshotStateProof, currency.CurrencyIncrementalSnapshot, CurrencySnapshotEvent]] = None
}
