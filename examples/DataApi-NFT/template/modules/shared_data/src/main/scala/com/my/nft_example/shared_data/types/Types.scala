...
object Types {
  ...
  case class NFT(id: Long, collectionId: String, owner: Address, uri: String, name: String, description: String, creationDateTimestamp: Long, metadata: Map[String, String])

  @derive(decoder, encoder)
  case class Collection(id: String, owner: Address, name: String, creationDateTimestamp: Long, nfts: Map[Long, NFT])
  @derive(decoder, encoder)
  ...
  @derive(decoder, encoder)
  case class NFTUpdatesCalculatedState(collections: Map[String, Collection]) extends DataCalculatedState
}
