...
object Main
  ...
  override def dataApplication: Option[BaseDataApplicationL1Service[IO]] = Option(BaseDataApplicationL1Service(new DataApplicationL1Service[IO, PollUpdate, State] {
    ...
    override def validateData(oldState: State, updates: NonEmptyList[Signed[PollUpdate]])(implicit context: L1NodeContext[IO]): IO[DataApplicationValidationErrorOr[Unit]] = MainData.validateData(oldState, updates)(context.securityProvider)
    ...
    override def combine(oldState: State, updates: NonEmptyList[Signed[PollUpdate]])(implicit context: L1NodeContext[IO]): IO[State] = MainData.combine(oldState, updates)
